import data from "../training.json" with { type: "json" };

let trainings = data.trainings;

function totalWeightLifted(trainings) {
    let training = "";
    let occurrences = "";
    let totalWeight = 0;
    ...
    for (let h = 0; h < trainings.length; h++) {
        ...
        for (let i = 0; i < ex.length; i++) {
            ...
            for (let j = 0; j < totalByExercise.length; j++) {
                totalWeight = totalWeight + totalByExercise[j].series * totalByExercise[j].repeat * totalByExercise[j].weight;
            }
        }
    }
